...
export const SupplierModule: React.FC = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 glass-panel p-5">
        <div>
          <h2 className="text-base font-bold text-slate-800 flex items-center gap-2">
            ...
            <span>{t('Supplier & Equipment Repair Vendor Directory')}</span>
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            {t('Manage tool manufacturers, calibration laboratories, and equipment service suppliers.')}
          </p>
        </div>
        ...
        {(activeRole === 'ADMIN' || activeRole === 'WAREHOUSE_MANAGER') && (
          ...
          >
            ...
            <span>{t('Register Vendor Supplier')}</span>
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {suppliers.length === 0 ? (
          <div className="col-span-full glass-panel p-8 text-center text-slate-400 text-xs">
            {t('No suppliers registered yet. Click "Register Vendor Supplier" to add suppliers.')}
          </div>
        ) : (
          suppliers.map((sup) => (
            <div key={sup.id} className="glass-card p-5 space-y-3">
              <h3 className="font-bold text-base text-slate-800">{sup.companyName}</h3>
              <p className="text-xs text-brand-700 font-semibold">{sup.services || t('General Tool Supplier')}</p>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={t('Register Vendor Supplier')}>
