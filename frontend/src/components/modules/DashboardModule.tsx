    ...
    <div className="space-y-5">
      ...
      <div className="glass-panel p-6 bg-gradient-to-r from-brand-700 to-brand-500 border-0 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-brand-100 mt-1">
            Real-time digital custody, preventative maintenance, calibration alerts, and asset depreciation.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          subtitle={`Acquisition: ${formatCurrency(totalOriginalValue)}`}
        ...
        <StatCard
          ...
          subtitle={`${Math.round((availableCount / (totalAssetsCount || 1)) * 100)}% ready for checkout`}
        ...
        <StatCard
          ...
          value={issuedCount}
          subtitle="Currently checked out in field"
          icon={<ArrowLeftRight className="w-5 h-5" />}
          accentColor="from-cyan-500 to-blue-500"
        />
        <StatCard
          title="Fleet Book Value"
          value={formatCurrency(totalCurrentValue)}
          subtitle={`Depreciation: ${formatCurrency(totalOriginalValue - totalCurrentValue)}`}
        ...
        <div className="glass-panel p-5 space-y-3">
          <div className="flex items-center justify-between pb-3 border-b border-surface-200">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-sm text-slate-800">Active Service & Repair Orders</h3>
            </div>
          ...
          <div className="space-y-2">
            {serviceOrders.length === 0 ? (
              <p className="text-xs text-slate-400 py-4 text-center">No active service orders</p>
            ) : (
        ...
        <div className="glass-panel p-5 space-y-3">
          <div className="flex items-center justify-between pb-3 border-b border-surface-200">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-sm text-slate-800">Calibration Due (Next 30 Days)</h3>
            </div>
          ...
          <div className="space-y-2">
            {expiringCalibrations.length === 0 ? (
              <div className="p-4 bg-emerald-50 border border-emerald-100 rounded-lg text-center">
                ...
                <p className="text-xs text-emerald-700">All instruments are fully calibrated.</p>
              </div>
            ) : (
              expiringCalibrations.map((cal) => (
                <div key={cal.id} className="p-3 bg-purple-50 border border-purple-100 rounded-lg flex items-center justify-between text-xs">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">Cert: {cal.certificateNumber}</p>
                  </div>
                  <div className="text-right">
                    <span className="text-[11px] font-bold text-red-600 block">Due: {cal.nextCalibrationDate}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="glass-panel p-5">
          <h3 className="font-bold text-sm text-slate-800 mb-4">Asset Status Distribution</h3>
          <div className="h-60">
        ...
        <div className="glass-panel p-5">
          <h3 className="font-bold text-sm text-slate-800 mb-4">Equipment by Category</h3>
          <div className="h-60">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="glass-panel p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-sm text-slate-800">Recent Asset Register</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-surface-50 border-b border-surface-200 text-[11px] uppercase tracking-wider text-slate-400">
                <th className="px-4 py-3 font-semibold">Asset No.</th>
          ...
          {assets.length === 0 && (
            <p className="text-xs text-slate-400 text-center py-8">No assets registered yet.</p>
          )}
        </div>
      </div>
